"""Custos de Peças e Serviços: análise dos gastos por agente causador etc.

Trabalha sobre a aba GASTOS com filtro de período (Data NF), empresa, tipo de
custo (Grupo Aplicação), fornecedor e agente causador. Gera KPIs, gráficos
(agente causador e top fornecedores) e relatório PDF.
"""
import pandas as pd
...
from ui_helpers import bar, base_carregada, colorir_barras, converter_datas, dataframe_estilizado, fmt_brl, fmt_int, kpi_cols, opcoes, plot_click, prompt_sem_base, rotular_barras, set_filtro, set_page, sidebar_acoes, theme_fig
...
# Prepara o DataFrame: só notas com valor, empresa normalizada e agente causador.
df = gastos().copy()
df = df[df["valor"].notna()]
df["empresa"] = df["locadora"].fillna("Não informado")
df["agente_causador"] = df["classe_manut"].fillna("Não classificado").str.upper()

# Limites do período disponível (para o date_input e o range do filtro).
...
# Aplica o filtro de período e os demais filtros laterais.
...
with col1:
    # Valor por agente causador com participação percentual e rótulos.
...
with col2:
    # Top 12 fornecedores (barra clicável filtra o fornecedor).
...
# Detalhamento das notas (mais recentes primeiro).
